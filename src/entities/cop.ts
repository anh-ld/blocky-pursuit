...
/* Cop tiers — visual escalation by level. 0: patrol (1-2), 1: urban (3), 2: SWAT (4-5), 3: Master (6+). */
type ICopTier = {
...
/* SWAT mini-boss tier — own materials so silhouette reads as "different threat" even through fog. */
...
/* Shared geometries — one set for all cops. Same proportions → GPU buffers live once for the session. */
...
const COP_WHEEL_GEO = new THREE.CylinderGeometry(UNIT * 0.76, UNIT * 0.76, UNIT * 0.8, 14);
COP_WHEEL_GEO.rotateZ(Math.PI / 2);
...
/* Per-step scratch reused across preStep. Physics runs single-threaded, values don't outlive one call. */
...
/* Drive both shared siren-light materials from one time source so all cops stay in phase. Called per frame. */
...
export class Cop {
  ...
  /* SWAT mini-boss flag — bigger, hit harder, immune to EMP. Read for bonus payout; skipped in empBlast(). */
  isSwat: boolean;
  isBounty: boolean;
  ...
  constructor(
    ...
    /* SWAT: heavier + extra accel — feels like a tank. Top speed only nudged so it can still be outrun. */
    if (isSwat) {
    ...
    this.preStepCallback = () => {
      ...
      /* Arcade friction — tighter grip. Re-read _localVel after applyLocalForce (can mutate body.velocity). */
      this.body.vectorToLocalFrame(this.body.velocity, _localVel);
  ...
  applyDamage(impactSpeed: number): number {
    ...
    this.damageMarker.scale.set(this.damageTier === 1 ? 0.8 : 1.15, 1, this.damageTier === 1 ? 0.85 : 1.05);
